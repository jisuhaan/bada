...
추천 바다(날씨 요약 및 상세 페이지 총괄, 로직 설계, 기상 데이터 수집 및 가공, 데이터 최적화) / 순위 바다(순위 페이지 로직 설계, 데이터 소스 통합), 회원 관리(회원 가입 프로세스 보완, 중복 방지 유효성 검사, 보안성과 사용자 경험 개선) / javascript 데이터 가공 전반 (클라이언트 측 데이터 계산 로직, 실시간 데이터 가공 및 연동) / api 통합 및 데이터 처리 담당 / PPT 제작 담당
　
